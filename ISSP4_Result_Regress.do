...
use "issp_religion_sample2_mz.dta", clear    
    foreach v in $x $c {
    	drop if `v'==.
    } //will have a change in sample if different y is used

  ta Male Year, col //age 20 to 59
  
  la var married2 "Married (ref: Single)"
  la var married3 "Sep/Div/Wid"
  la var nchildc2 "1 child (ref: No child)"
  la var nchildc3 "2 children"
  la var nchildc4 "3 children"
  la var working2 "Part-time working (ref: Full-time working)"
  la var working3 "Not working"
  /*
  la var educ1 "Lowest formal education (ref: Higher secondary)"
  la var educ2 "Secondary education"
  la var educ4 "College"
  la var educ5 "University or above"
  */
  la var educ2 "Secondary (ref: <Sec Education)"
  la var educ3 ">Second Education"
  la var urbanc1 "Urban, big city (ref: suburban city,big town)"
  la var urbanc3 "Town or small city"
  la var urbanc4 "Rural, village"
  la def year 0 "2002" 1 "2012"
  la val Yeardum year

save "issp_religion_sample3_mz.dta",replace 
